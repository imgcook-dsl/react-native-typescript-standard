module.exports = function(schema, option) {
  ...
  const parseTsStyle = (className, style, componentName)=> {
    const styleObj = parseStyle(style, componentName);
    return `const ${className}Style: ${componentName}Style = ${JSON.stringify(styleObj)};`
  }
  ...
  //get the rn class type
  const getClassComponentType = (type)=> {
    if(type === 'text') {
      return 'Text'
    } else if(type === 'image') {
      return 'Image'
  ...
  const generateRender = (schema) => {
    ...
    const ComponentType = getClassComponentType(type);

    const className = schema.props && schema.props.className;
    let classString = "";

    if (className) {
      classString = ` style={${className}Style}`;
      style[className] = parseTsStyle(className, schema.props.style, ComponentType);
